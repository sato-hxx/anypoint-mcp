...
interface CacheConfig {
  /** デフォルトTTL（ミリ秒、デフォルト: 60秒） */
  defaultTTL?: number;

  /** 最大キャッシュサイズ（バイト、デフォルト: 50MB） */
  maxCacheSize?: number;
...
class TTLBasedCacheManager implements Cacheable {
  ...
  constructor(config: CacheConfig = {}) {
    ...
    this.config = {
      defaultTTL     : config.defaultTTL      ?? 60 * 1000       , // 60秒
      ...
      resourceTTL    : config.resourceTTL     ?? {},
    };

    this.ttlPathMatcher = new PathTrie<number>();
    for (const [path, ttl] of Object.entries(this.config.resourceTTL)) {
      this.ttlPathMatcher.insert(path, ttl);
    }
